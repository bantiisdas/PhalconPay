...
function findFavoriteByAddress(address: string): FavoriteWallet | undefined {
  if (!address) return undefined;
  const { wallets } = useFavoritesStore.getState();
  return wallets.find((w) => w.address === address);
}

function favoriteToIconType(
  favorite: FavoriteWallet | undefined,
  fallback: TransactionIconType,
): TransactionIconType {
  if (!favorite?.avatarType) return fallback;
  switch (favorite.avatarType) {
    case "coffee":
      // Use avatar-style icon so it matches Favorites (cafe outline),
      // not the generic coffee payment icon used in mocks.
      return "avatar";
    case "exchange":
      return "swap";
    case "wallet":
...
function storedToRow(t: StoredTransaction): TransactionRow {
  ...
  switch (t.kind) {
    case "send_sol":
      {
        const favorite = findFavoriteByAddress(t.toAddress);
        return {
          id: t.id,
          title: favorite?.name ?? truncateAddr(t.toAddress),
          amount: `${t.amount} ${t.symbol}`,
          type: "sent",
          time,
          iconType: favoriteToIconType(favorite, "sol"),
          iconColor: favorite?.avatarColor,
          favoriteAvatarType: favorite?.avatarType,
        };
      }
    case "send_token":
      {
        ...
        return {
          ...
          type: "sent",
          time,
          iconType: favoriteToIconType(favorite, "payment"),
          iconColor: favorite?.avatarColor,
          favoriteAvatarType: favorite?.avatarType,
        };
      }
    ...
    case "swap_and_send":
      {
        const favorite = findFavoriteByAddress(t.toAddress);
        const toLabel = favorite?.name ?? truncateAddr(t.toAddress);
        return {
          id: t.id,
          title: `Swap & Send to ${toLabel}`,
          amount: `${t.amount} ${t.symbol}`,
          type: "sent",
          time,
          iconType: favoriteToIconType(favorite, "swap"),
          iconColor: favorite?.avatarColor,
          favoriteAvatarType: favorite?.avatarType,
        };
      }
