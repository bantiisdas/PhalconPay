...
export default function AddWalletScreen() {
  ...
  const canSubmit =
    address.trim().length > 0 && nickname.trim().length > 0;
  ...
  const previewWallet: Pick<FavoriteWallet, 'name' | 'avatarType' | 'avatarColor'> = {
    name: nickname.trim() || '?',
    avatarType: avatarType ?? undefined,
        ...
        <Card style={styles.card}>
          ...
          <View style={styles.iconRow}>
            {AVATAR_OPTIONS.map((opt) => (
              ...
              >
                {opt.type === undefined ? (
                  <View style={[styles.iconCircle, { backgroundColor: avatarColor }]}>
